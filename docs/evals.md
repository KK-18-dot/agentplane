...
Each trial gets a fresh workdir (`seed/` copied, `git init`, one commit) under the results directory, so `changed` in the HANDOFF is meaningful and a bad run cannot touch your repository. Results are appended to `results.jsonl`; `report.md` is regenerated at the end. The command exits 1 if any trial failed.
...
- **Exclude non-capability failures.** `quota-exhausted`, `auth-required`, and `timeout` are infrastructure signals; filter them from pass rates when comparing routes (`status` is in every row).
